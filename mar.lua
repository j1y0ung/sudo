...
local function dbClick(event)
	if event.target.id == 'apr' then
		print("4월클릭")
		db:exec([[UPDATE clue SET opened = 'TRUE' WHERE name = '캘린더' AND content = '4월';]]) 
	end
end
...
function scene:create( event )
	...
	--전, 후달
	local goF = display.newText("2월", 50, 130, font, 45)
	goF:setFillColor(0)
	goF.name = "3-2"
	
	local goA = display.newText("4월", 650, 130, font, 45)
	goA:setFillColor(0)
	goA.id = "apr"
	goA.name = "3-4"
	...
	local function go_A(event)
		dbClick(event)
		composer.removeScene("mar")
		composer.gotoScene("apr")
		print("4월")
	end
	...
	--일정
	local promise = display.newText("월급", 540, 625)
	promise.size = 30
