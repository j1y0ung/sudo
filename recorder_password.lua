...
function scene:create( event )
   ...
   local logo_img = display.newImageRect("ui/pass_ico_rec.png", 300, 300) 
  ...
  if data == 'TRUE' then
      composer.removeScene("recorder_password")
      composer.gotoScene("recorder")
  end

   local count = 0
   local passwd = ""
   local answer = "0202" 	--딸이 죽은날
   function input( event )
      ...
      if count == 4 then
         if passwd == answer then
            composer.removeScene(composer.getSceneName( "current" ))
            composer.gotoScene("recorder")
            local sql = [[UPDATE status SET opened = 'TRUE' WHERE name = '녹음';]]
            db:exec(sql)
         else
            for i = 0, 3, 1 do
