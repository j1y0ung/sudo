...
function scene:create( event )
   ...
   local logo_img = display.newImage(logo_sheet, 6)
   ...
   circle[3].alpha = 0
	...
	if data == 'TRUE' then
    	composer.removeScene(composer.getSceneName( "current" ))
		composer.gotoScene("bIntro")
	end

   local count = 0
   local passwd = ""
   local answer = "2325" 	--비밀갤러리 성경구절
   function input( event )
      ...
      if count == 4 then
         if passwd == answer then
    		composer.removeScene(composer.getSceneName( "current" ))
            composer.gotoScene("bIntro")
            local sql = [[UPDATE status SET opened = 'TRUE' WHERE name = '밴드';]]
			db:exec(sql)
         else
            for i = 0, 3, 1 do
